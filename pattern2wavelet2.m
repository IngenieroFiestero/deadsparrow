function [ imageAndPattern ] = pattern2Wavelet2( pattern,image,lvl )
...
if(patternSize(1) ~= tamAux(1) || patternSize(2) ~= tamAux(2))
    ...
    x=x + floor(difX/2);
    y=y + floor(difY/2);
    pattern2 = zeros(tamAux(1) , tamAux(2));
...
%levels{lvl,3} = levels{lvl,3} +  levels{lvl,3}*pattern2;
%levels{lvl,2} = levels{lvl,2} +  levels{lvl,2}*pattern2;
%levels{lvl,4} = levels{lvl,4} +  levels{lvl,4}*pattern2;
myLevel= levels{lvl,1};
lastCA(:,:,1) = myLevel(:,:,1) +  myLevel(:,:,1).*pattern2;
lastCA(:,:,2) = myLevel(:,:,2) +  myLevel(:,:,2).*pattern2;
lastCA(:,:,3) = myLevel(:,:,3) +  myLevel(:,:,3).*pattern2;
